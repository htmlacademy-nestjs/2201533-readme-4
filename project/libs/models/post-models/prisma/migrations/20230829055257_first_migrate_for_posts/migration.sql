...
CREATE TABLE "followers" (
    ...
    CONSTRAINT "followers_pkey" PRIMARY KEY ("followed","follower")
);
...
-- CreateIndex
CREATE INDEX "posts_pub_date_idx" ON "posts"("pub_date");
...
CREATE VIEW "titles" AS
  SELECT titles.title, posts.id, posts.type, posts.content_id FROM
    (SELECT 'text' as type, title, id from texts
      UNION
    SELECT 'video' as type, title, id from videos) as titles
      LEFT JOIN
    (SELECT id, type, content_id FROM posts) as posts
      ON titles.type = posts.type AND titles.id = posts.content_id
